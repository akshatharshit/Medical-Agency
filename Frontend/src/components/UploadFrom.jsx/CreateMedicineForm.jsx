...
export default function CreateMedicineForm() {
  ...
  const validate = () => {
    ...
    if (!formData.imageFile) errs.imageFile = "Image is required";
    if (!formData.packageSize.trim()) errs.packageSize = "Package Size is required";
    if (!formData.mrp.trim() || isNaN(formData.mrp) || Number(formData.mrp) <= 0)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success("Medicine created successfully!");  // show success toast
      navigate("/admin");
    } catch (err) {
      console.error("Create medicine error:", err);
      toast.error(err?.message || "Failed to create medicine. Please try again."); // show error toast
    } finally {
      setLoading(false);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          <label className="block font-semibold mb-1">
            Medicine Image <span className="text-error">*</span>
          </label>
          <div
            ...
            {!imagePreview ? (
              <>
                <svg
                  ...
                  className="h-12 w-12 text-base-content/50 mb-2"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={2}
                ...
                <p className="text-base-content/70">Drag & drop an image here, or click to select</p>
                <p className="text-sm text-base-content/50 mt-1">(PNG, JPG, GIF)</p>
              </>
            ) : (
              <img
                src={imagePreview}
          ...
          {errors.imageFile && <p className="text-error mt-1">{errors.imageFile}</p>}
        </div>
        ...
        <div>
          ...
          {errors.inStockNumber && (
            <p className="text-error mt-1">{errors.inStockNumber}</p>
          )}
        </div>
